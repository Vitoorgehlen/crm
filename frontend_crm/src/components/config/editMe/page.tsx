...
export default function EditMe({u, onUpdate}: ConfigMeProps) {
  ...
  return (
    <form className={styles.overlay}>
      ...
      <button 
      className={`${isOpenEmail ? styles.btnSettingActive : styles.btnSetting}`}
      type="button"
      onClick={(e) => setIsOpenEmail(prev => !prev)}
      >
        <h3>Alterar seu e-mail {isOpenEmail 
        ? <MdKeyboardArrowDown /> 
        : <MdKeyboardArrowRight /> }</h3>
      </button>
      {isOpenEmail && <div className={styles.labelsSetting}>
      ...
      <button 
      className={`${isOpenPassword ? styles.btnSettingActive : styles.btnSetting}`}
      type="button"
      onClick={(e) => setIsOpenPassword(prev => !prev)}
      >
        <h3>Alterar sua senha {isOpenPassword 
        ? <MdKeyboardArrowDown /> 
        : <MdKeyboardArrowRight /> }</h3>
      </button>
      {isOpenPassword && <div className={styles.labelsSetting}>
        <input
        className={styles.labelSetting}
        type="password"
      ...
      onClick={handleEditMe} type="button">
