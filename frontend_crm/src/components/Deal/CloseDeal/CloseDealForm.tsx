...
import { PaymentMethod, Deal, CloseDealPayload, CommissionSplit, CloseDealFormProps } from "@/types"; 
...
export default function CloseDealForm({
    ...
    const [companyUsers, setCompanyUsers] = useState<Array<{ id: number; name?: string }>>([]);
    const [loadingUsers, setLoadingUsers] = useState(false);
  
    const [propertyValue, setPropertyValue] = useState<number>(Number(deal.propertyValue ?? 0));
    const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>(deal.paymentMethod ?? "FINANCING");
    const [cashValue, setCashValue] = useState<number>(Number(deal.cashValue ?? 0));
    const [fgtsValue, setFgtsValue] = useState<number>(Number(deal.fgtsValue ?? 0));
    const [financingValue, setFinancingValue] = useState<number>(Number(deal.financingValue ?? 0));
    const [creditLetterValue, setCreditLetterValue] = useState<number>(Number(deal.creditLetterValue ?? 0));
    const [installment, setInstallment] = useState(false);
    const [installmentValue, setInstallmentValue] = useState<number>(Number(deal.installmentValue ?? 0));
    const [installmentCount, setInstallmentCount] = useState<number>(Number(deal.installmentCount ?? 0));
    const [bonusInstallmentValue, setBonusInstallmentValue] = useState<number>(Number(deal.bonusInstallmentValue ?? 0));
    const [bonusInstallmentCount, setBonusInstallmentCount] = useState<number>(Number(deal.bonusInstallmentCount ?? 0));
    
    const [commissionAmount, setCommissionAmount] = useState<number>(Number(deal.commissionAmount ?? 0));
    ...
    useEffect(() => {
        ...
        setPropertyValue(
            Number(initialPropertyValue ?? deal.propertyValue ?? 0)
        );
        setCommissionAmount(
        ...
        setFinancingValue(Number(initialFinancingValue ?? deal.financingValue ?? 0));
        setCreditLetterValue(Number(initialCreditLetterValue ?? deal.creditLetterValue ?? 0));
        setInstallmentValue(Number(initialInstallmentValue ?? deal.installmentValue ?? 0));
        setInstallmentCount(Number(initialInstallmentCount ?? deal.installmentCount ?? 1));
        setBonusInstallmentValue(Number(initialBonusInstallmentValue ?? deal.bonusInstallmentValue ?? 0));
        setBonusInstallmentCount(Number(initialBonusInstallmentCount ?? deal.bonusInstallmentCount ?? 1));
    ...
    function computedAmountFor(index: number) {
        ...
        if (splitMethod === 'percentage') {
            ...
            const valueString = valueNumber?.toLocaleString(
                'pt-BR', { 
    ...
    const balanceText = isPositive ? `Sobrando${diffFormatted}` : isNegative ? `Faltando${diffFormatted}` : `${diffFormatted}`;
    function validate(): string | null {
        if (!commissionAmount || Number.isNaN(commissionAmount) || commissionAmount <= 0) {
        ...
        if (roundedTotal !== roundedCom) {
            return `A soma dos valores dos splits deve ser igual ao valor da comissão (${roundedCom.toFixed(2)}).`;
        }
        }

        for (const s of splits) {
            if (!s.isCompany && !s.userId) return 'Cada split deve apontar para um usuário ou para a empresa.';
            if (splitMethod === 'percentage' && (s.percentage === undefined || s.percentage === null)) return 'Preencha todos os percentuais.';
            if (splitMethod === 'amount' && (s.amount === undefined || s.amount === null)) return 'Preencha todos os valores dos splits.';
        }
    ...
    return (
        <div className={styles.overlay} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <h2 className={`${styles.balance} ${isPositive ? styles.positive : isNegative ? styles.negative : styles.ok}`}>                    
                    {balanceText}
                </h2>
                ...
                <>
                    ...
                    <div className={styles.payment}>
                        ...
                        value={cashValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        onChange={(e) => {
                            const numeric = Number(e.target.value.replace(/\D/g, ""))/ 100;
                        ...
                        type="text"
                        placeholder="FGTS"
                        value={fgtsValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        onChange={(e) => {
                            const numeric = Number(e.target.value.replace(/\D/g, ""))/ 100;
                    ...
                    <div className={styles.nameBtnInstallment}>
                        <button 
                            className={`${styles.btnInstallment} ${installment ? styles.btnInstallmentActive : ''}`}
                    ...
                    {installment && <>
                        ...
                        <div className={styles.payment}>
                            ...
                            value={installmentValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            onChange={(e) => {
                                const numeric = Number(e.target.value.replace(/\D/g, ""))/ 100;
                            ...
                            type="text"
                            placeholder="Reforço"
                            value={bonusInstallmentValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            onChange={(e) => {
                                const numeric = Number(e.target.value.replace(/\D/g, ""))/ 100;
                            ...
                            onChange={(e) => setBonusInstallmentCount(Number(e.target.value))}
                            />
                        </div>
                    </>}
                </>
                ...
                <>
                    ...
                    <div className={styles.payment}>
                        ...
                        value={financingValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        onChange={(e) => {
                                const numeric = Number(e.target.value.replace(/\D/g, ""))/ 100;
                ...
                <>
                    ...
                    <div className={styles.payment}>
                        ...
                        <input
                            ...
                            value={creditLetterValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            onChange={(e) => {
                                const numeric = Number(e.target.value.replace(/\D/g, ""))/ 100;
                ...
                <div className={styles.boxCommissionShare}>
                    <div className={styles.splitCommissionTitle}>
                        ...
                        <button
                            className={`
                                ${styles.btnCommission} ${splitMethod === 'percentage' 
                                ? styles.btnActive : ''}`}
                            type="button"
                        ...
                        <button
                            className={`
                                ${styles.btnCommission} ${splitMethod === 'amount' 
                                ? styles.btnActive : ''}`}
                            type="button"
                        ...
                        <button className={styles.addSplit} type="button" onClick={addSplit}>
                            <MdOutlineAddCircle />
                        </button>
                    </div>
                        ...
                        <div key={i} className={styles.border}>
                            <div className={styles.boxSplitCommission}>
                                ...
                                >
                                    ...
                                    {companyUsers.map(u => 
                                    <option key={u.id} value={u.id}>{u.name ?? `Usuário ${u.id}`}</option>
                                    )}
                                </select>
                                ...
                                <>
                                    ...
                                    placeholder="%"
                                    />
                                    <h3 className={styles.textSplit}> ≈ R$ {computedAmountFor(i)}</h3>
                                </>
                                ) : (
                                <>
                                    <input
                                ...
                                <button className={styles.removeSplit}
                                    type="button" 
                    ...
                    <div style={{ marginTop: 8 }}>
                        ...
                        <h3>Somatório: R$ {totalAmounts.toFixed(2)} / Comissão R$ {commissionAmount.toFixed(2)}</h3>
                        )}
                    </div>
                </div>
                ...
                <h6>Atualizado a última vez por: {deal?.updater?.name ?? "—"} ·{" "}
                    {deal?.updatedAt ? formatDateForCards(deal.updatedAt) : "—"}</h6>
                <h6> Criado por: {deal?.creator?.name ?? "—"} ·{" "}
                    {deal?.createdAt ? formatDateForCards(deal.createdAt) : "—"}</h6>
                </div>
            </div>
        </div>
    );
