...
import { PaymentMethod, CloseDealPayload, CommissionSplit, CloseDealFormProps, DealStepType, DocumentationCost, Note, DEAL_STEP_TYPE_LABEL, WORKFLOW_BY_METHOD } from "@/types"; 
...
export default function ClosedDeal({
    ...
    const [isOpenDocCost, setIsOpenDocCost] = useState<number | undefined>(undefined);
    ...
    function buildPayload(): CloseDealPayload {
        const normalizedSplits = splits.map(s => {
            const amount = splitMethod === 'percentage'
    ...
    async function handleChangeStep(e: React.FormEvent, step: string) {
        ...
        if (step === 'next' && isLastStep) {
            ...
            if (hasUnpaidCommission) {
                setError('Não é possível finalizar a negociação com comissões pendentes.');
                return;
            }
        }
  ...
  function calculateStepPosition() {
    ...
    async function handleDeleteDocCost(docId?: number) {
        if (typeof docId !== 'number') return
        const confirmDelete = window.confirm(`Tem certeza que deseja excluir essa documentação?`)
    ...
    async function handleEditNote(noteId?: number) {
        ...
        try {
            ...
            const data = await res.json();
            setNote(prev => 
                prev.map(item => (item.id === noteId ? data : item))
            );
    ...
    async function handleDeleteNote(noteId?: number) {
        if (typeof noteId !== 'number') return
        const confirmDelete = window.confirm(`Tem certeza que deseja excluir essa nota?`)
        if (!confirmDelete) return;
                            ...
                            <div key={i} className={styles.border}>
                                <div className={styles.boxSplitCommission}>
                                    <div className={styles.paymentBox}>
                                        ...
                                        : 
                                        <h4>
                                            {companyUsers.find(user => user.id === s.userId)?.name ?? 'Empresa'}
                                        </h4>
                                        }
                                    </div>       
                                            ...
                                            !s.isPaid ? 
                                                <div className={styles.paymentBox}>
                                                    ...
                                                    <div className={styles.percentCommission}>
                                                        ...
                                                        placeholder="%"
                                                        />
                                                        <h3 className={styles.textSplit}> ≈ {computedAmountFor(i)}</h3>
                                                    </div>
                                                </div>
                                            : <div className={styles.paymentBox}>
                                                <h3>Valor</h3>
                                        ...
                                        !s.isPaid ? <div className={styles.paymentBox}>
                                                <h3>Valor</h3>
                                                <input
                                                type="text"
                                                style={{ width: 100 }}
                                                ...
                                                placeholder="Valor"
                                                />
                                            </div> : <div className={styles.paymentBox}>
                                                <h3>Valor</h3>
                                    ...
                                    <div className={styles.paymentBox}>
                                        ...
                                        <div className={styles.paymentBox}>
                                            ...
                                            <button
                                                className={`
                                                    ${styles.btnIsPaid} ${s.isPaid ? styles.btnIsPaidActive : ''}`}
                                                type="button"
                                                onClick={() => updateSplit(i, { isPaid: !s.isPaid })}
                                            >
                                                {s.isPaid ? <><GiCheckMark /> Recebido!</> : <>Receber</>}
                                            </button>
                                        </div>
                    ...
                    <div className={styles.docCostSection}>
                        ...
                        <div className={styles.addDoc}>
                            {isOpenDocCost ? <>
                                ...
                                />
                                <button className={styles.btnSave} onClick={handleAddDocCost} disabled={!docCostLabel.trim()}>
                                    <RiSave3Fill />
                                </button>
                            </>}
                        </div>
                        ...
                        <div className={styles.docList}>
                            {docCost.length === 0 && <p>Nenhuma documentação encontrada.</p>}
                            {docCost.map ( doc => (
                    ...
                    <div className={styles.noteSection}>
                        ...
                        <div className={styles.addNote}>
                            {isOpenNote ? <>
                                ...
                                <button className={styles.btnSave} onClick={handleAddNote} disabled={!newNote.trim()}>
                                    <RiSave3Fill />
                                </button>
                            </>}
                        </div>
                        ...
                        <div className={styles.noteList}>
                            {note.length === 0 && <p>Nenhuma nota do cliente encontrada.</p>}
                            {note.map ( note => (
