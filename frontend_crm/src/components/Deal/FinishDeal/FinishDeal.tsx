...
import { calculateDuration, formatDateForCards, formatDateForFinish } from "@/utils/dateUtils";
import { CommissionSplit, CloseDealFormProps, DealStepType, DocumentationCost, Note, DEAL_STEP_TYPE_LABEL } from "@/types"; 
...
export default function FinishDeal({
    ...
    useEffect(() => {
        ...
        async function fetchDocumentationCost() {
            try {
                ...
                const data = await res.json();
                setDocCost(data);
                const total = data.reduce((sum: number, item: DocumentationCost) =>
    ...
    return (
        <div className={styles.overlay} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            <div className={`${styles.modal} ${deal.status === 'CLOSED' ? styles.modalRed : ''}`} onClick={(e) => e.stopPropagation()}>
                ...
                <div className={styles.modalLeft}>
                    ...
                    <div className={styles.paymentTitle}>
                        ...
                        <div className={styles.paymentBox}> 
                            ...
                            <h4>{deal.paymentMethod === 'CREDIT_LETTER' && 'Carta de Crédito'}</h4>
                        </div>
                        ...
                        {splits.map((s, i) => (
                            <div key={i} className={styles.border}>
                                <div className={styles.boxSplitCommission}>
                                    <h3>{s.isCompany ? 'Imobiliária' : (s.userId != null 
