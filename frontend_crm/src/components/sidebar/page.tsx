        ...
        <div className={styles.contentbar} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.sidebarActions}>
            ...
            <Link href="/arquivados" onClick={() => setIsOpen(false)}>
              <div
                className={`${pathname === "/arquivados" && styles.active} ${styles.icons}`}
              >
                <FaRegFolderOpen className={styles.logos} />
                <p className={styles.linkText}>Arquivados</p>
              </div>
            </Link>
            <Link href="/negociacoes" onClick={() => setIsOpen(false)}>
              <div
                className={`${pathname === "/negociacoes" && styles.active} ${styles.icons}`}
              >
                <TbHomeSearch className={styles.logos} />
                <p className={styles.linkText}>Negociações</p>
              </div>
            </Link>
            <Link href="/fechados" onClick={() => setIsOpen(false)}>
              <div
                className={`${pathname === "/fechados" && styles.active} ${styles.icons}`}
              >
                <TbContract className={styles.logos} />
                <p className={styles.linkText}>Fechados</p>
              </div>
            </Link>
            <Link href="/finalizados" onClick={() => setIsOpen(false)}>
              <div
                className={`${pathname === "/finalizados" && styles.active} ${styles.icons}`}
              >
                <GrCompliance className={styles.logos} />
                <p className={styles.linkText}>Finalizados</p>
              </div>
            </Link>
            <Link href="/comissoes" onClick={() => setIsOpen(false)}>
              <div
                className={`${pathname === "/comissoes" && styles.active} ${styles.icons}`}
              >
                <TbPigMoney className={styles.logos} />
                <p className={styles.linkText}>Comissões</p>
              </div>
            </Link>
            {permissions.includes("ALL_DEAL_DELETE") && (
              <Link href="/desempenho" onClick={() => setIsOpen(false)}>
                <div
                  className={`  ${pathname === "/desempenho" && styles.active} ${styles.icons}`}
                >
                  <MdOutlineQueryStats className={styles.logos} />
                  <p className={styles.linkText}>Desempenho</p>
                </div>
              </Link>
            )}
            {permissions.includes("ALL_DEAL_DELETE") && (
              <Link href="/requisicoes" onClick={() => setIsOpen(false)}>
                <div
                  className={`  ${pathname === "/requisicoes" && styles.active} ${styles.icons}`}
                >
                  <FiUserX className={styles.logos} />
                  <p className={styles.linkText}>Requisições</p>
                </div>
              </Link>
            )}
          </div>
