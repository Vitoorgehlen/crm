...
import { Client, CreateSchedulePayload, Schedule, ScheduleFormProps } from "@/types";
...
export default function ScheduleForm({
  ...
  const [loading, setLoading] = useState<'read' | 'create' | 'del' | 'addNote' | null >(null);
  ...
  function formatDateForInput(date: Date | undefined): string {
    ...
    useEffect(() => {
        if (isOpen) {
            if (schedule) {
                ...
                setNewDate(schedule.reminderAt ? new Date(schedule.reminderAt) : undefined)
            } else {
    ...
    async function handleCreate(payload: CreateSchedulePayload) {
        try {     
            ...
            if (!response.ok) throw new Error(data.error || 'Erro ao salvar a agenda');
    ...
    async function handleEdit(payload: CreateSchedulePayload) {
        ...
        try {     
            ...
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Erro ao editar a agenda');
    ...
    const handleDelete = async (schedule: Schedule) => {
        const confirmDelete = window.confirm(`Tem certeza que deseja excluir o compromisso?`)
                    ...
                    <div className={styles.modalLeft}>
                        ...
                        <div className={styles.btns}>
                            {schedule &&
                                <button 
                                type="button" 
                                className={`${styles.btnFilter} ${newFinish ? styles.btnFilterActive : ''}`}
