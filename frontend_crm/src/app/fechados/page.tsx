...
import { Client, Deal, CloseDealPayload, DEAL_STEP_TYPE_LABEL, PAYMENT_METHOD_LABEL, WORKFLOW_BY_METHOD } from '@/types/index'
...
export default function Deals() {
  ...
  const fetchDealsData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchDeals(API!, token, { team: teamDeals, search, status: ['CLOSED'] });
      setDeals(data);
    } catch (err: unknown) {
      console.error(err);
    } finally {
    ...
    <div className={styles.page}>
      <main className={styles.main}>
        ...
        <div className={styles.boxSteps}>
          {(
            Object.keys(WORKFLOW_BY_METHOD) as PaymentMethod[]
          ).map((method) => {
            const steps = WORKFLOW_BY_METHOD[method];
            const dealsForMethod = deals.filter(d => d.paymentMethod === method);
              ...
              <section key={method} className={styles.methodSection}>
                <header className={styles.methodHeader}>
                  ...
                  <h4 className={styles.methodCount}>
                    {dealsForMethod.length} {dealsForMethod.length === 1 ? 'negociação' : 'negociações'}
                  </h4>
                </header>

                <div className={styles.methodWorkflow}>
                  {steps.map((step, idx) => {
                    const columnDeals = dealsForMethod.filter(d => d.currentStep === step);
                    return (
                      <div className={styles.column} key={String(step)}>
                        <h5 className={styles.stepName}>{DEAL_STEP_TYPE_LABEL[step]}</h5>
                        <div className={styles.dealList}>
                          {columnDeals.map((dealItem) => (
                            ...
                            >
                              <h3>{dealItem.client?.name || 'Cliente não informado'}</h3>
                              { teamDeals && 
                                <h6>{dealItem.creator?.name || 'Usuário não encontrado'}</h6>
                              }
                            </button>
                          ))}
