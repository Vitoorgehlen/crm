...
export default function Deals() {
  ...
  const fetchDealsData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchDeals(API!, token, { team: teamDeals, search, status: ['POTENTIAL_CLIENTS'] });
      setDeals(data);
    } catch (err: unknown) {
      console.error(err);
    } finally {
  ...
  function onClientUpdatedHandler(updatedClient: Client) {
    setDeals(prev => prev.map(d => {
      ...
      if (d.client.id === updatedClient.id) {
        return { ...d, client: { ...d.client, isPriority: updatedClient.isPriority } };
      }
      return d;
    }));
  }
    ...
    <div className={styles.page}>
      <main className={styles.main}>
        <div className={styles.header}>
          <h1>{teamDeals ? 'Negociações da equipe' : 'Negociações'}</h1>
        </div>
        ...
        <div>
          ...
          .filter(statusObj => deals.some(deal => deal.statusClient === statusObj.dbValue))
          .map(statusObj => {
            ...
            return (
              ...
              key={statusObj.dbValue}>
                ...
                <div className={styles.dealList}>
                  {deals          
                    .slice().sort((a, b) => (b.client?.isPriority ? 1 : 0) - (a.client?.isPriority ? 1 : 0))
                    ...
                    .map((deal, index) => (
                      <button
                        ...
                        type="button"
                        className={`
                          ${deal.client?.deleteRequest ? styles.dealDelete : 
                            deal.deleteRequest ? styles.dealDelete : styles.deal
                          }`}
                      ...
                      >
                        ...
                        { teamDeals && 
                          <h6>{deal.creator?.name || 'Usuário não encontrado'}</h6>
                        }
                      </button>
                    ))
                  }
