...
import { MdOutlinePowerOff, MdOutlineEdit, MdDeleteForever, MdAutoDelete } from "react-icons/md";

import { FaUserEdit, FaUnlockAlt, FaUsersCog, FaUserPlus } from "react-icons/fa";
...
export default function Config() {
  ...
  const deleteUser = async (UserDelete: User) => {
    const confirmDelete = window.confirm(`Tem certeza que deseja excluir ${UserDelete.name}?`)
    ...
    <div className={styles.page}>
      <main className={styles.main}>
        ...
        <div className={styles.headerContent}>
          ...
          <button 
            className={`${isConfigUser ? styles.btnSettingActive : styles.btnSetting}`}
            onClick={(e) => {
          ...
          {permissions.includes('USER_UPDATE') && 
            <button 
              className={`${isConfigTeam ? styles.btnSettingActive : styles.btnSetting}`}
              onClick={(e) => {
          ...
          {user?.role ==='ADMIN' && 
            <button 
              className={`${isOpenPermissions ? styles.btnSettingActive : styles.btnSetting}`}
              onClick={(e) => {
                setIsConfigUser(false);
                setIsConfigTeam(false);
                        ...
                        <div key={u.id} className={styles.editUser}>
                          ...
                          <div className={styles.editUserLabels}>
                            ...
                            <div className={styles.box}>
                              <h5>{RoleLabels[u.role || 'Cargo não encontrado']}</h5>
                            </div>
                          </div>
                          :<>
                          <div className={styles.editUserLabels}>
