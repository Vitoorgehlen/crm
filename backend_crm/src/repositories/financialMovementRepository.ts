...
export async function getFinancialMovement(userId: number, start: string, end: string) {
    ...
    if (!canReadExpense) throw new Error('Você não tem permissão para ler as despesas');
    ...
    const startDate = new Date( Number(startYear), Number(startMonth) - 1, 1, 0, 0, 0, 0);

    const endDate = new Date( Number(endYear), Number(endMonth), 0, 23, 59, 59, 999 );
