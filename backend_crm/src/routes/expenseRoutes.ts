...
import { processAutoPayments } from "../cron/processAutoPayments";
...
router.get("/auto-payments-expenses", cronOnly, async (req, res) => {
  try {
    await processAutoPayments();

    res.status(200).json({
      success: true,
      message: "Despesas pagas automaticamente com sucesso",
    });
  } catch (error) {
    console.error("[CRON] Erro ao pagar despesas automaticamente:", error);
    res.status(500).json({
      error: "Erro ao pagar despesas automaticamente",
    });
  }
});
...
router.post("/expense/", loginRequired, async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      error:
        err instanceof Error ? err.message : "Erro ao adicionar a despesa.",
    });
  }
});
...
router.get("/expense/", loginRequired, async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      error: err instanceof Error ? err.message : "Erro ao ler a despesa.",
    });
  }
});
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    const { id: userId } = user;
    if (!userId)
      return res.status(400).json({ error: "Usuário não identificada." });
    ...
    try {
      const updatedDeal = await updateExpense(id, data, userId);
      return res.status(200).json(updatedDeal);
    } catch (err) {
      console.error(err);

      return res.status(400).json({
        error: err instanceof Error ? err.message : "Erro ao editar a despesa.",
      });
    }
  },
);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const updatedDeal = await updateRecurringStatus(
        id,
        isRecurringActive,
        userId,
      );
...
router.delete("/expense/:id", loginRequired, async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({
      error: err instanceof Error ? err.message : "Erro ao excluir a despesa.",
    });
  }
});
