...
import {
  upsertDocumentationDefaults,
  upsertDocumentationCustom,
  deleteDocumentationCustom,
  getDocumentationDefault,
  getDocumentationDefaultSU,
} from "../repositories/previousDocumentationRepository";
...
router.put(
  ...
  async (
    ...
    if (!docData)
      return res
        .status(400)
        .json({ error: "Erro ao criar documentação padrão." });
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Erro ao criar o documentação personalizada." });
    }
  },
);

router.get(
  "/documentation-default-SU",
  superUserOnly,
  async (
    req: Request & { superUser?: { id: number; email: string } },
    res: Response,
...
router.put("/documentation-custom", loginRequired, async (req, res) => {
  ...
  if (!docData)
    return res
      .status(400)
      .json({ error: "Erro ao editar documentação padrão." });
...
router.delete("/documentation-custom", loginRequired, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao excluir documentação personalizada." });
  }
});
